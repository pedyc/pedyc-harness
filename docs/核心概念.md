...
| Scope Enforcement     | 范围约束 | 已实现（M7）          | `evaluateFiles`、`refusedFiles`、`describeFileViolations`；规则 id `allowedProductPaths` 与 `protectedPaths` |
| Verification          | 独立验证 | 已实现                | `requiredChecks` 逐个执行；结果类型 `VerificationCheck`              |
| Verification Evidence | 验证证据 | **目标(M8)**          | 今天只有 `VerificationCheck`（`command` / `result` / `details`），**没有**信任等级、退出码、耗时或输出摘要 |
| Review                | 结果审查 | 已实现（布尔）        | Reviewer 阶段返回 `approved: boolean`；**结构化 Findings 是 M8**    |
...
判定的最终解释权在 Harness：`reviewerApproved` 要求 reviewer 批准**且**没有任何被拒文件，
即 Agent 的 `approved` 不能抵消 Harness 自己发现的问题。
