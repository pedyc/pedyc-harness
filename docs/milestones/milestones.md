...
但已提前完成。**当前实际位置：M15、M16 已由 M12 发布为 1.2.0（声明式配置层）；M7（策略可执行）
与 M8（独立验证与证据链）都已完成，由 M14 发布为 1.3.0（发布准备已就绪，等待推送 npm）。**
实际推进顺序是：

```mermaid
flowchart TD
  M0["M0"] --> M1 --> M2 --> M3 --> M4 --> M5 --> M6
  M6 --> M11
  M11 --> M15 --> M16["M16<br/>已完成"]
  M16 --> M12["M12(发布 1.2.0:声明式配置层)<br/>已发布"]
  M12 --> M7["M7(策略可执行)<br/>已完成"] --> M8["M8(独立验证与证据链)<br/>已完成"] --> M17 --> M9 --> M10
  M7 --> M14["M14(发布 1.3.0:治理执行)<br/>发布准备就绪"]
  M8 --> M14
  M13["M13(可移植 Provider,在 1.2.0 基线之后,可随时插入)"]
  M18["M18"] --> M19["M19(在 M17 之后)"]
  M20["M20(生态命令,可随时插入)"]
  M21["M21(代码 Preset,在 M8 与 M17 之后)"]
```
...
```mermaid
flowchart TD
  A["M15–M16<br/>决定配置从哪里读"]
  B["M7<br/>才能基于稳定来源实现强制执行"]
  C["M17<br/>才能基于 M7 实际字段定义合并语义"]
  D["M9<br/>才能把生效配置与 provenance 写进 Run Record"]
  A --> B --> C --> D
```
...
| 里程碑 | 目标                              | 状态   |
| ------ | --------------------------------- | ------ |
| M7     | 让策略真正可执行（Policy Engine） | 已完成 |
| M8     | 建立独立验证与结构化证据链        | 已完成 |
| M9     | 建立执行轨迹与审计记录            | 计划中 |
| M10    | 建立审批门与人工介入机制          | 计划中 |

> M7 依赖 M16 提供的配置来源（**已具备**），M9 依赖 M17 提供的生效配置与 provenance，因此本阶段
> 的启动时间晚于 M15–M16；M15–M16、M7 与 M8 都已落地，M9 是治理阶段的下一步。
