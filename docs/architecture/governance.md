...
| ② Review     | Task、验收标准、Evidence、实际改动、Harness 的 Finding    | Findings（结构化）        | 外部 Agent（`reviewer` 阶段）与 Harness 自己的检查 |
...
flowchart TD
  ...
  C --> D["外部 Tester 判定证据"]
  D --> E["越界检查 + Harness 一致性 Finding + 外部 Reviewer"]
  E --> F["Findings 处置<br/>severity → action"]
  F --> G["全部通过 → 结束；可修复 → 回 Coder 重试；否则终止"]
  G -.重试.-> A
```

> **目标（M8）** 目标形态在这条链路上还差一处：`requiredChecks` 之外增加 Preset 注册的
> Evidence Provider（M21 提供注册面）。已经落地的是 `requiredChecks` 产生的结构化 Evidence
> （命令、退出码、耗时、输出摘要、信任等级）、「没有证据不得批准」，以及 Reviewer 输出与 Harness
> 自产的结构化 Findings——由 Gate 按 `severity → action` 处置。见
...
```text
通过 = 所有闸门 pass && Tester approved && 存在可判定的 Evidence
    && 范围判定允许 && 没有 reject 处置的 Finding && Reviewer approved
```

要点：

* Agent 的 `approved` 不能替代 Harness 的执行结果，也不能抵消越界；
* 越界判定由 Harness 做，交给被检查的一方没有意义：它是独立的一步（`scope` 阶段记录与
  `RunResult.scope`），Reviewer 批准与否只记录 Reviewer 自己的裁定；
* 没有任何可判定的 Evidence 时 Reviewer 不得批准，因此 `requiredChecks` 为空的项目不会因为
  「空真」而悄悄通过——它会以 `agent_error` 结束并说明缺少证据；
* Finding 的处置与 Reviewer 的裁定也是两件事：一条 `reject` Finding 会让运行不通过，即使 Reviewer
  投了赞成票。

### 5.2 Findings 的处置

**已实现。** 每条 Finding 按其 rule id 在 Policy 中解析出 severity 与 action，默认映射为
`error → reject`、`warning → review`、`info → report`。三者取最严格者：规则声明的级别、
`policy.rules` 的覆盖、Finding 自己的声明。

处置只决定"这次判定算不算通过"，不改变证据本身。关键推论：

* `info` 不影响判定，只进入记录；
* `warning` 需要 Reviewer 明确确认，未确认即不通过：Harness 自己产生的 warning 需要 Reviewer 报告
  同一条 rule id，Reviewer 自己报告的 warning 天然已确认；
* `error` 不通过；
* 没人声明的 rule id 不判定，运行以 `agent_error` 结束并列出该 id——猜一个级别出来等于替规则实现
  编造语义。

### 5.3 回流语义

**已实现。** 重试通道有两条，都携带结构化内容：Tester 失败后把上一轮 `verification` 交回 Coder，
以及按 Findings 回流：

| 情况                             | 行为                                       |
| -------------------------------- | ------------------------------------------ |
| 可修复的 `error` / `warning`     | 回 Coder 重试，携带上一轮 Findings（结构化） |
| 不可修复的 `error`               | 终止运行（`policy_violation`）              |
| 越界改动、受保护路径命中         | 立即终止，不重试                           |
| 没有任何 Evidence                | Reviewer 不得批准                          |
| 重试次数用尽                     | 以上限终止（`max_iterations`）              |

"可修复"由 Finding 自身声明，不由 Agent 事后解释。任何情况下都不允许把自然语言总结当作回流内容：
Coder 收到的是 Finding 对象本身。一次尝试产生的 claim 不会留到下一次尝试去定罪——claims 与产生它们
的那一轮 diff 配对比较。
...
前三级的证据参与判定；`agent-claimed` 只能作为线索进入上下文，`judgingEvidence` 会在判定前把它
过滤掉。`review-derived` 与 `agent-claimed` 分开记录：前者是 Harness 主动派发、由另一个执行者产出的
观察，后者是被检查方的自我描述。等级本身写进记录，否则审计无法回答"这条结论凭什么可信"。

今天实际产生的等级是 `harness-executed`（requiredChecks 闸门与 Tester 自述分别记为
`harness-executed` 与 `agent-claimed`）；`analyzer-derived` 与 `review-derived` 随结构验证与语义
审查一起到来（M8 的后续工作项）。
...
| 能力                        | 当前                                             | 目标                            |
| --------------------------- | ------------------------------------------------ | ------------------------------- |
| 确定性证据                  | 结构化 Evidence（命令、退出码、耗时、摘要）        | Preset 注册的 Evidence Provider（M21） |
| 分析器扩展                  | 内置分析器（`css.duration`、`json.property`）+ 一致性检查 | Preset 注册分析器（M21）      |
| 证据信任等级                | 四级来源标记；判定只看非 `agent-claimed`          | 保持不变                        |
| 语义判断                    | Reviewer 与语义检查都返回结构化 Findings          | 规则与审查定义来源可扩展到 Preset（M21） |
| 语义审查触发                | 规则命中触发；按 role 合批，一轮一次调用           | 增加启发式分数触发（M8 后续）    |
| 模型来源                    | 由 role 直接配置，语义调用复用同一协议             | Preset 声明需求、Harness 选择 provider（ADR-005） |
| 预算与禁用                  | `maxSemanticCalls` 耗尽时 warning 跳过、error fail-closed；`--semantic=disabled` 入记录 | 成本算术归 [成本权衡](../tradeoffs/成本权衡.md) |
| 处置                        | `severity → action` 映射 + `policy.rules` 收紧 + 越界一票否决 | `conflicts` 记录（M17） |
| 回流                        | 上一轮 `verification` 与结构化 Findings 回 Coder  | 保持不变                        |
| Reviewer 独立性             | `agents.json` 可声明 `source`，运行记录 `independence` | 可强制（M19 安全模型）        |
| 记录                        | `Evidence[]` + `Finding[]` + `scope` + `semantic` + `violations` + `output.json` | 统一 RunRecord，与 RunResult 分离（M9） |
...
| Finding 为 `reject` 且可修复   | 回 Coder 重试，携带 Finding 本身         |
| `warning` 未被 Reviewer 确认   | 不通过：按 `reject` 处置                 |
| rule id 无人声明               | 该条不判定，运行以 `agent_error` 结束并列出 id |
| Finding 引用了不存在的路径     | 该引用被丢弃，Finding 本身保留           |
| Agent 声称的改动不在 diff 中   | 记一条 `change.claimed-file-missing` warning，交 Reviewer 确认 |
