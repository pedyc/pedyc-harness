...
| `scope` | 验证之后、判定之前 | 比对实际改动与允许范围 | `runtime/diff-inspector.ts` + `policy-engine.ts` |
...
* Reviewer 的通过条件**包含**范围检查:越界改动一票否决。

> **目标(M8)** `scope` 从 Reviewer 步骤里提出来,成为独立报告的判定,而不是藏在 `details` 文本里。
...
| `timeout` | Harness(**目标 M7**;今天 `agentTimeoutMs` 未被读取) |
| `policy_violation` | Harness(命令策略或范围判定) |
| `agent_error` | Harness(Provider 进程失败或响应不合法) |
| `cancelled` | 用户(**目标**;今天没有取消管线) |
...
| 产物 | 回答 | 今天 |
| ---------- | ---------------- | ------------------------------------------------------------ |
| `RunResult` | 这次任务最终怎么样 | `output.json`,很薄,**没有 `runId` 与终止原因** |
| `RunRecord` | 这次任务发生了什么 | `.harness/runs/<runId>/` 的 input / policy / iteration-n-verification / output(**M9 才统一**) |
...
> 判定)、`evidence.ts`(证据与信任等级)、`run-record.ts`(审计记录)。`executor.ts` 继续做阶段
> 编排者。事件总线与独立 Monitor **暂不引入**:它们会变成没有行为的空壳,而本仓库已经因为同样的
> 理由移除过一个算法目录(见[文档规范](../CONVENTIONS.md) §3)。
...
| 能力 | 当前 | 目标 |
| ------------------ | -------------------------------------------- | ---------------------------------------------- |
| 编排阶段 | 固定四阶段 + 散落的加载/校验 | 两套词汇 + `PhaseRecord.kind`(ADR-006) |
| Preflight | 分散,没有名字 | 独立阶段,失败不启动 Agent |
| 生效配置 | 有序预设列表 | `EffectiveHarnessConfig` + provenance(M17) |
| 范围判定 | 藏在 Reviewer 内部,只体现为 `details` | 独立报告的判定(M8) |
| 证据 | `{command, result, details}` | 结构化 Evidence + 四级信任等级(M8) |
| 终止原因 | 折叠成 `passed`/`failed` | `termination` + `status` + `verdict`(ADR-006) |
| 审计记录 | 四个散落文件 | 统一 RunRecord,与 RunResult 分离(M9) |
| 超时与取消 | `agentTimeoutMs` 未被读取;无取消管线 | 接入命令执行(M7) |
