...
const valueAfter = (name: string, fallback: string | null): string | null => {
...
const manifestOf = (dir: string): { version: string } =>
...
const main = (): void => {
  ...
  const whoami = run('npm', ['whoami', '--registry', registry as string], root)
  ...
  const profile = run('npm', ['profile', 'get', '--json', '--registry', registry as string], root)
  let tfaMode = 'unknown'
  try {
  ...
  for (const { name } of packages) {
    const existing = run('npm', ['view', `${name}@${version}`, 'version', '--registry', registry as string], root)
  ...
  if (dryRun) {
    for (const { dir, name } of packages) {
      const result = run('pnpm', ['publish', '--dry-run', '--access', 'public', '--no-git-checks', '--registry', registry as string], join(root, dir))
  ...
  for (const { dir, name } of packages) {
    const args = ['publish', '--access', 'public', '--no-git-checks', '--registry', registry as string]
